...
moduleToJs (Module _ coms mn _ imps exps foreigns decls) foreign_ =
  ...
  accessorInteger :: Integer -> AST -> AST
  accessorInteger index = AST.Indexer Nothing $ AST.NumericLiteral Nothing $ Left index
  ...
  valueToJs' e@(Abs (_, _, _, Just IsTypeClassConstructor) _ _) =
    let
      args = unAbs e
      object = AST.ObjectLiteral Nothing $ map assign args
      block = AST.Block Nothing [AST.Return Nothing $ object]
    in return $ AST.Function Nothing Nothing (map identToJs args) block
    where
    unAbs :: Expr Ann -> [Ident]
    unAbs (Abs _ arg val) = arg : unAbs val
    unAbs _ = []
    assign :: Ident -> (PSString, AST)
    assign name = (mkString $ runIdent name, var name)
  ...
  bindersToJs ss binders vals = do
    ...
    where
      ...
      valueError s _                          = accessorInteger 0 $ AST.Var Nothing s
  ...
  binderToJs' varName done (ConstructorBinder (_, _, _, Just (IsConstructor ctorType fields)) _ q bs) = do
    ...
    go ((field, binder) : remain) done' = do
      ...
      return (AST.VariableIntroduction Nothing argVar (Just $ accessorInteger field $ AST.Var Nothing varName) : js)
